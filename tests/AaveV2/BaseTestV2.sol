...
abstract contract BaseTestV2 is Test {
  ...
  function test_latestAnswerRel() public virtual {
    for (uint i = 0; i < adapterParams.preRequisiteAdapters.length; i++) {
      address preAdapter = GovV3Helpers.deployDeterministic(adapterParams.preRequisiteAdapters[i]);

      if (i == adapterParams.preRequisiteAdapters.length - 1) {
        bool isCapped = IPriceCapAdapterStable(preAdapter).isCapped();
        assertFalse(isCapped);
      }
    }

    ICLSynchronicityPriceAdapter adapter = ICLSynchronicityPriceAdapter(
